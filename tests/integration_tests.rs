...
use neuro_fed_node::openai_proxy::types::{Choice, Message, OpenAiRequest, OpenAiResponse, Usage};
...
async fn test_story_pc_confidence_growth_and_fallback() -> anyhow::Result<()> {
    ...
    println!(
        "Initial confidence: {:.4}, novelty: {:.4}",
        initial_stats.confidence_score, initial_stats.novelty_score
    );
    ...
    assert!(
        learned_stats.confidence_score >= 0.0,
        "Confidence should be valid"
    );
    assert!(
        learned_stats.novelty_score >= 0.0,
        "Novelty should be valid"
    );

    // Log the results to show the PC is functioning
    println!(
        "✅ PC inference after learning: Confidence: {:.4}, Novelty: {:.4}",
...
async fn test_story_sleep_phase_dreaming() -> anyhow::Result<()> {
    ...
    let pc = Arc::new(RwLock::new(PredictiveCoding::new_with_device(
        pc_config,
        device.clone(),
    )?));
    ...
    let sleep_manager =
        SleepManager::new(pc, decoder.clone(), dict.clone(), episodic_memory.clone());

    // TRIGGER DREAM PHASE
    sleep_manager
        .process_sleep_cycle()
        .await
        .map_err(|e| anyhow::anyhow!("Sleep phase error: {}", e))?;
    ...
    assert!(
        new_dict_size > initial_dict_size,
        "Failed to discover new chunks! Dictionary size: {}",
        new_dict_size
    );
    ...
    assert_eq!(
        decoder_rows, new_dict_size,
        "Thought Decoder weight matrix should match dictionary size!"
    );

    println!(
        "✅ Dream phase successfully consolidated new chunk. Decoder size is now {}",
        decoder_rows
    );
    Ok(())
}
...
async fn test_story_semantic_caching() -> anyhow::Result<()> {
    ...
    let req_a = OpenAiRequest {
        ...
        messages: vec![Message {
            role: "user".into(),
            content: serde_json::json!("Write a for loop"),
            name: None,
        }],
        ..Default::default()
    };
    ...
    // Store in cache
    cache
        .add_to_cache(req_a, mock_resp.clone(), emb_a.clone(), None)
        .await?;
    ...
    let req_b = OpenAiRequest {
        ...
        messages: vec![Message {
            role: "user".into(),
            content: serde_json::json!("Create python for loop"),
            name: None,
        }],
        ..Default::default()
    };
    ...
    assert!(
        ...
        "Semantic Cache failed to identify similar request!"
    );
    ...
    let req_c = OpenAiRequest {
        ...
        messages: vec![Message {
            role: "user".into(),
            content: serde_json::json!("Explain the French Revolution"),
            name: None,
        }],
        ..Default::default()
    };
    ...
    assert!(
        miss_result.is_none(),
        "Semantic Cache hallucinated a hit on completely unrelated data!"
    );

    println!(
        "✅ Semantic Cache successfully triggered on similar vectors and rejected orthogonal vectors."
    );
    Ok(())
}
