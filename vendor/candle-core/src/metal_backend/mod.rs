...
impl BackendDevice for MetalDevice {
    ...
    fn new(ordinal: usize) -> Result<Self> {
        ...
        let device = if devices.is_empty() && ordinal == 0 {
            Device::system_default()
                .ok_or_else(|| MetalError::Message("no Metal device found".to_string()))?
        } else if ordinal < devices.len() {
            devices.swap_remove(ordinal)
        } else {
            return Err(MetalError::Message(format!(
                "no Metal device found for ordinal {ordinal}"
            ))
            .into());
        };
