...
impl ThoughtDecoder {
    ...
    pub fn resize_vocab(&mut self, new_vocab_size: usize) -> Result<(), PCError> {
        ...
        tracing::info!(
            "Expanded ThoughtDecoder vocabulary from {} to {}",
            old_vocab_size,
            new_vocab_size
        );
        Ok(())
    ...
    fn decode_sequence_with_rules(
        ...
        let scale_factor = if anchor_norm > 1e-6 {
            target_norm / anchor_norm
        } else {
            1.0
        };
        ...
        for _step in 1..=max_steps {
            ...
            for (_, raw_score, seq, h_t, is_done, seen_mask) in &beams {
                ...
                let mut indexed: Vec<(f32, usize)> = log_probs_vec
                    .into_iter()
                    .enumerate()
                    .map(|(i, v)| (v, i))
                    .collect();
                ...
                let top_k_log_probs_vec: Vec<f32> =
                    indexed[..top_k].iter().map(|&(v, _)| v).collect();
                let top_k_indices_vec: Vec<u32> =
                    indexed[..top_k].iter().map(|&(_, i)| i as u32).collect();
    ...
    pub fn decode_from_hierarchy(
        ...
        let flattened: Vec<Tensor> = level_beliefs
            .iter()
            .map(|t| t.flatten_all())
    ...
    pub fn train_step(
        ...
        let target_norm = (belief_dim as f64).sqrt();
        let scale_factor = if belief_norm > 1e-6 {
            target_norm / belief_norm
        } else {
            1.0
        };
        ...
        let mut dw_gate_stack_acc =
            Tensor::zeros_like(self.w_gate_stack.as_tensor())?.contiguous()?;
        ...
        for state in states.into_iter().rev() {
            ...
            let grad_vocab_tensor = Tensor::from_vec(
                grad_vocab,
                (1, self.w_vocab.shape().dims()[0]),
                &self.device,
            )?;
            ...
            let sig_deriv = (&state.update_gate
                * &(state.update_gate.ones_like()?.sub(&state.update_gate)?))?;
            ...
            let d_combined =
                (d_pre_sig.matmul(&w_gate_chunks[0])? + d_pre_tanh.matmul(&w_gate_chunks[1])?)?;
        ...
        let (dw_vocab_final, dw_gate_final) = if total_norm > clip_val {
            ...
            (
                dw_vocab_acc.affine(scale, 0.0)?,
                dw_gate_stack_acc.affine(scale, 0.0)?,
            )
        } else {
        ...
        self.w_vocab.set(
            &self
                .w_vocab
                .as_tensor()
                .add(&dw_vocab_final.affine(-lr, 0.0)?)?,
        )?;
        self.w_gate_stack.set(
            &self
                .w_gate_stack
                .as_tensor()
                .add(&dw_gate_final.affine(-lr, 0.0)?)?,
        )?;
    ...
    pub fn set_weights(&mut self, w_gate: &[f32], w_vocab: &[f32]) -> Result<(), PCError> {
        ...
        if w_vocab.len() >= v_rows * v_cols && w_gate.len() == g_rows * g_cols {
            let new_vocab =
                Tensor::from_slice(&w_vocab[0..v_rows * v_cols], (v_rows, v_cols), &self.device)?;
            ...
            tracing::warn!(
                "Decoder DB dimension mismatch. Skipping load. (Expected Vocab: {}, Got: {})",
                v_rows * v_cols,
                w_vocab.len()
            );
        }
...
mod tests {
    ...
    fn test_gru_gate_logic_learns_sequence() -> Result<(), PCError> {
        ...
        assert!(
            final_loss < 2.0,
            "Decoder failed to converge, final loss is high: {}",
            final_loss
        );
    ...
    fn test_resize_vocab_preserves_gradient_flow() -> Result<(), PCError> {
        ...
        assert_ne!(
            new_rows_before[0], new_rows_after[0],
            "Weights for new token 8 did not update after training."
        );
    ...
    fn test_top_k_pruning_in_beam_search() -> Result<(), PCError> {
        ...
        // Clamp the gate stack so the hidden state remains deterministic for the test.
        let zeros = Tensor::zeros(
            decoder.w_gate_stack.shape(),
            candle_core::DType::F32,
            &device,
        )?;
        decoder.w_gate_stack.set(&zeros)?;
        ...
        assert!(
            [10, 20, 30].contains(&token_id),
            "Top-k pruning produced unexpected token {}",
            token_id
        );
    ...
    fn test_fused_matmul_updates_weights() -> Result<(), PCError> {
        ...
        assert_ne!(
            ...
            "Fused gate weights (w_gate_stack) did not update during training."
        );
        Ok(())
    }
    ...
    fn test_bptt_optimization_correctness() -> Result<(), PCError> {
        ...
        assert!(
            loss2 < loss1,
            "BPTT optimization appears to have broken gradient descent."
        );
        Ok(())
    }
    ...
    fn test_bptt_actually_learns_a_sequence() -> Result<(), PCError> {
        ...
        assert!(
            ...
            "Decoder loss did not decrease after training. Initial: {}, Final: {}",
            initial_loss,
            last_loss
        );
        ...
        assert!(
            loss_decrease > 0.05,
            "Loss decrease too small: {:.2}% (initial: {}, final: {})",
            loss_decrease * 100.0,
            initial_loss,
            last_loss
        );
        ...
        assert_ne!(
            w_vocab_before, w_vocab_after,
            "w_vocab was not updated during training."
        );
        assert_ne!(
            w_gate_stack_before, w_gate_stack_after,
            "w_gate_stack was not updated during training."
        );
    ...
    fn test_decoder_gradient_flow() -> Result<(), PCError> {
        ...
        assert!(
            loss_2 < loss_1,
            "Loss must decrease. Gradients are likely zero. Check initialization."
        );
        Ok(())
    }
    ...
    fn test_escape_from_dead_loss() -> Result<(), PCError> {
        ...
        for step in 0..30 {
            ...
            if (loss - 2.0795).abs() < 1e-4 {
                panic!(
                    "Decoder stuck in dead network state (loss = {:.4}) at step {}. This indicates uniform probability distribution bug.",
                    loss, step
                );
            }
        }
        ...
        info!(
            "✅ Escape-from-dead-loss test passed. Loss trajectory: {:.4} -> {:.4}",
            losses[0],
            losses.last().unwrap()
        );
        Ok(())
    ...
    fn test_global_gradient_clipping() -> Result<(), PCError> {
        ...
        assert!(
            total_norm > clip_val,
            "Test setup failed: total norm is not large enough to trigger clipping."
        );
        ...
        assert!(
            ((vocab_norm_after + gate_norm_after).sqrt() - clip_val as f32).abs() < 1e-6,
            "Global norm clipping failed to scale the total norm correctly."
        );
...
mod reasoning_behavior_tests {
    ...
    fn test_decoder_repetition_penalty_breaks_loops() -> Result<(), PCError> {
        ...
        assert!(
            unique_tokens.len() > 1,
            "Repetition penalty failed! Decoder looped on the same token: {:?}",
            seq
        );
...
mod decoder_persistence_tests {
    ...
    fn test_get_weights_returns_correct_shapes() -> Result<(), PCError> {
        ...
        assert_eq!(
            gate_weights.len(),
            expected_gate_len,
            "Gate weights length mismatch: expected {} ({}x{}), got {}",
            expected_gate_len,
            combined_dim,
            combined_dim,
            gate_weights.len()
        ...
        assert_eq!(
            vocab_weights.len(),
            expected_vocab_len,
            "Vocab weights length mismatch: expected {} ({}x{}), got {}",
            expected_vocab_len,
            vocab_size,
            belief_dim,
            vocab_weights.len()
        ...
        println!(
            "✅ Get weights test passed: gate_len={}, vocab_len={}",
            gate_weights.len(),
            vocab_weights.len()
        );
        Ok(())
    ...
    fn test_check_if_random_detects_untrained_decoder() -> Result<(), PCError> {
        ...
        assert!(
            is_random,
            "Freshly initialized decoder should be detected as random"
        );
    ...
    fn test_check_if_random_after_training() -> Result<(), PCError> {
        ...
        assert!(
            !is_random,
            "Trained decoder should not be detected as random"
        );
