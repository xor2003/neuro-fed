...
impl BackendClient {
    ...
    pub async fn send_to_ollama(&self, req: &OpenAiRequest) -> Result<OpenAiResponse, ProxyError> {
        ...
        let response_text = response.text().await.map_err(|e| {
            ProxyError::InvalidResponse(format!("Failed to read Ollama response: {}", e))
        })?;
        ...
        let response_body =
            serde_json::from_str::<OpenAiResponse>(&response_text).map_err(|e| {
                tracing::error!(
                    "Failed to parse Ollama JSON response. Raw body: {}",
    ...
    pub async fn send_to_fallback(
        ...
        tracing::info!(
            "📤 Outgoing Remote Request: {}",
            serde_json::to_string_pretty(&final_req).unwrap_or_default()
        );

        let response = builder.json(&final_req).send().await.map_err(|e| {
            tracing::error!(
                "❌ Network Error: check your internet or TLS features! {:?}",
                e
            );
        ...
        let status = response.status();
        let response_text = response.text().await.map_err(|e| {
            ProxyError::InvalidResponse(format!("Failed to read fallback response: {}", e))
        })?;
        ...
        let response_body =
            serde_json::from_str::<OpenAiResponse>(&response_text).map_err(|e| {
                tracing::error!(
                    "Failed to parse JSON response from remote. Raw body: {}",
                    response_text.chars().take(500).collect::<String>()
                );
