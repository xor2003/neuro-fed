...
pub fn create_router(state: Arc<OpenAiProxy>) -> Router {
    Router::new()
        .route(
            "/v1/models",
            routing::get(|State(_): State<Arc<OpenAiProxy>>| async move {
                Ok::<Json<serde_json::Value>, axum::http::StatusCode>(Json(
        ...
        .route(
            "/v1/chat/completions",
            routing::post(handlers::handle_chat_completion),
        )
        .route(
            "/v1/completions",
            routing::post(handlers::handle_completion),
        )
