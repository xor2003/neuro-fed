...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let local_engine = Arc::new(RwLock::new(
        MLEngine::new("models/example-model.gguf", device_type.clone()).unwrap_or_else(|_| {
    ...
    let pc_hierarchy = Arc::new(RwLock::new(
        PredictiveCoding::new(pc_config).expect("Failed to create Predictive Coding hierarchy"),
    ));
