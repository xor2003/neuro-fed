...
fn main() {
    ...
    match NodeConfig::load_from_file("config.toml") {
        Ok(config) => {
            ...
            println!("Proxy config - OpenAI Base URL: {}", config.proxy_config.openai_base_url);
            println!("Proxy config - OpenAI API Key present: {}", config.proxy_config.openai_api_key.is_some());
