...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Non-contiguous matmul (100 iterations): {:?}",
        non_contig_duration
    );
    ...
    println!(
        "   Contiguous matmul (100 iterations): {:?}",
        contig_duration
    );
    ...
    let start = Instant::now();
    for t in 0..sequence_len {
        let token_emb = sequence_tensor.narrow(0, t, 1)?.reshape((dim, 1))?; // No contiguous() call
        let _ = token_emb.matmul(&other_tensor)?;
    }
    let unoptimized_duration = start.elapsed();
    println!(
        "   Unoptimized sequence processing: {:?}",
    ...
    println!(
        "4. Expected study session speed improvement: {}x",
        speedup.max(seq_speedup)
    );
